<nav class="navbar navbar-expand-md navbar-dark fixed-top" style="background-color:#6777EF;border-top:5px solid #4853a7;">
    <div class="container-fluid">
        <a href="/" class="navbar-brand">
            <i class="fa fa-graduation-cap" aria-hidden="true"></i> SMA ANTARIKSA
        </a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarCollapse"
            aria-controls="navbarCollapse" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <ul class="navbar-nav mr-auto mb-2 mb-md-0">
                <li class="nav-item">
                    <a href="" class="nav-link">
                        <i class="fa fa-book-open" arial-hidden="true"></i> BERITA
                    </a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link">
                        <i class="fa fa-calendar" aria-hidden="true"></i> AGENDA
                    </a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link">
                        <i class="fa fa-image" aria-hidden="true"></i> GALERI
                    </a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link">
                        <i class="fa fa-video" aria-hidden="true"></i> VIDEO
                    </a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link">
                        <i class="fa fa-phone" aria-hidden="true"></i> KONTAK
                    </a>
                </li>
            </ul>
        </div>
    </div>
</nav>
